...
const TAX_AGENT_INSTRUCTIONS = [
  ...
  '2. An ENGAGEMENT-YEAR is one client + one tax year + one program: T2 (federal),',
  '   AT1 (Alberta) or CO17 (Québec). It is ONE FILING, not one corporation-year.',
  '   `create_engagement_year { clientId, program, taxYearStart, taxYearEnd }`.',
  '   A corporation with an Alberta or Québec permanent establishment owes two',
  '   returns, and therefore needs two engagement-years. Do not enter the data',
  '   twice: the provincial engagement already holds the whole federal dataset',
  '   (the province is computed FROM the federal figures), so the',
  '   `create-companion-filing` action opens the federal one with those figures',
  ...
  '   save its balanceSheet + incomeStatement. If you have the .cor file from a',
  "   previously filed return, the `gifi` resource's `corImport` tool reads it —",
  '   the same figures, plus who the file is for; check that against the client',
  '   before saving anything from it.',
  ...
  '6. EXPORT: `prepare-cif` (T2), `prepare-netfile` or `prepare-rsi` (AT1), or',
  '   `prepare-co17` (CO17) renders the filing payload so a preparer can read it.',
  '   Each REFUSES on the wrong program — a provincial engagement computes the',
  '   federal figures but does not keep them, so a federal payload built from one',
  '   would be missing every federal amount. You CANNOT transmit.',
  '',
